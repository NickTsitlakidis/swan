...
export class SolanaNFTService {
    public readonly LAMPORT_MULTIPLIER = LAMPORTS_PER_SOL;

    public readonly ARWEAVE_UPLOAD_ENDPOINT = "https://us-central1-metaplex-studios.cloudfunctions.net/uploadFile";

    public async getAssetCostToStore(files: { size: number }[]) {
    ...
    public async mintNFT( connection: Connection, wallet: WalletSigner | undefined, endpoint: ENDPOINT_NAME,
        files: File[],
        metadata: {
            name: string;
            symbol: string;
