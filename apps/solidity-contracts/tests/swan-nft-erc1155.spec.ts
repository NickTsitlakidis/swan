...
import { ERC1155MixedFungible, InterfaceCheck } from "../typechain-types";

describe("ERC1155MixedFungible - Unit Tests", () => {
    const baseUri = "https://swan-nft/{id}";
    ...
    const nonFungibleTokenId = BigInt("57896044618658097711785492504343953926975274699741220483192166611388333031425");
    ...
    context("Mint function", () => {
        context("for non-fungible tokens", () => {
            ...
            it("signer should be able to mint their own tokens with custom URI", async () => {
                await expect(
                    deployedERC1155
                        .connect(deployerSignerA)
                        .mintNonFungibleWithURI(nonFungibleTokenTypeId, [deployerSignerA.address], "0x00", "testURI")
                ).to.emit(deployedERC1155, "TransferSingle");
        ...
        context("for fungible tokens", () => {
            ...
            it("non-deployer of contract should not be able to mint tokens", async function () {
                ...
                await expect(
                    deployedERC1155.connect(signerB).mintFungible(fungibleTokenTypeId, [signerB.address], [100], "0x00")
                ).to.be.reverted;
            ...
            it("non-signing address should not be able to mint tokens", async function () {
                ...
                await expect(
                    deployedERC1155.connect(ONE_ADDRESS).mintFungible(fungibleTokenTypeId, [ONE_ADDRESS], [100], "0x00")
                ).to.be.reverted;
                expect(await deployedERC1155.balanceOf(ONE_ADDRESS, fungibleTokenTypeId)).to.equal(0);
            });
        });
    ...
    context("Transfer function", () => {
        context("for non-fungible tokens", () => {
            beforeEach(async () => {
                ...
                expect(
                    await deployedERC1155.connect(deployerSignerA).balanceOf(signerB.address, nonFungibleTokenId)
                ).to.equal(0);
                expect(
                    await deployedERC1155.connect(deployerSignerA).balanceOf(signerC.address, nonFungibleTokenId)
                ).to.equal(0);
            });
            ...
            it("signer should be able to transfer their token to another signer", async () => {
                await expect(
                    deployedERC1155
                        .connect(deployerSignerA)
                        .safeTransferFrom(deployerSignerA.address, signerB.address, nonFungibleTokenId, 1, "0x00")
                ).to.emit(deployedERC1155, "TransferSingle");
                expect(
                    await deployedERC1155
                        .connect(deployerSignerA)
            ...
            it("signer should be able to transfer approved token on behalf of another signer", async () => {
                await expect(deployedERC1155.connect(deployerSignerA).setApprovalForAll(signerB.address, true)).to.emit(
                    deployedERC1155,
                    "ApprovalForAll"
                );
                await expect(
                    deployedERC1155
                        .connect(signerB)
                        .safeTransferFrom(deployerSignerA.address, signerC.address, nonFungibleTokenId, 1, "0x00")
                ).to.emit(deployedERC1155, "TransferSingle");
                expect(
                    await deployedERC1155
                        .connect(deployerSignerA)
        ...
        context("for fungible tokens", () => {
            beforeEach(async () => {
                ...
                expect(
                    await deployedERC1155.connect(deployerSignerA).balanceOf(signerB.address, fungibleTokenTypeId)
                ).to.equal(0);
                expect(
                    await deployedERC1155.connect(deployerSignerA).balanceOf(signerC.address, fungibleTokenTypeId)
                ).to.equal(0);
            });
            ...
            it("signer should be able to transfer their token to another signer", async () => {
                await expect(
                    deployedERC1155
                        .connect(deployerSignerA)
                        .safeTransferFrom(deployerSignerA.address, signerB.address, fungibleTokenTypeId, 20, "0x00")
                ).to.emit(deployedERC1155, "TransferSingle");
                expect(
                    await deployedERC1155
                        .connect(deployerSignerA)
            ...
            it("signer should be able to transfer approved token on behalf of another signer", async () => {
                ...
                await expect(
                    deployedERC1155
                        .connect(signerB)
                        .safeTransferFrom(deployerSignerA.address, signerC.address, fungibleTokenTypeId, 30, "0x00")
                ).to.emit(deployedERC1155, "TransferSingle");
                await expect(
                    deployedERC1155
                        .connect(signerB)
            ...
            it("signer should not be able to transfer token on behalf of another signer without approval", async () => {
                await expect(
                    deployedERC1155
                        .connect(signerB)
                        .safeTransferFrom(deployerSignerA.address, signerC.address, fungibleTokenTypeId, 1, "0x00")
                ).to.be.revertedWith("ERC1155: caller is not owner nor approve");
                expect(
                    await deployedERC1155
                        .connect(deployerSignerA)
    ...
    context("Burn function", () => {
        context("for non-fungible tokens", () => {
            ...
            it("signer should be able to burn more than 1 token", async () => {
                ...
                await expect(deployedERC1155.connect(deployerSignerA).burn(deployerSignerA.address, tokenId, 2, "0x00"))
                    .to.be.reverted;
        ...
        context("for fungible tokens", () => {
            ...
            it("signer should not be able to burn tokens owned by others", async function () {
                ...
                await expect(
                    deployedERC1155.connect(deployerSignerA).burn(signerB.address, fungibleTokenTypeId, 70, "0x00")
                ).to.be.reverted;

                expect(await deployedERC1155.connect(signerB).balanceOf(signerB.address, fungibleTokenTypeId)).to.equal(
                    100
                );
            });
        });
